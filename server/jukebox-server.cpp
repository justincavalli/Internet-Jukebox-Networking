...
std::vector<fs::path> find_mp3_files(const char *dir, bool find_info);
void event_loop(int epoll_fd, int server_socket, std::vector<fs::path> song_path, std::vector<fs::path> info_path);
...
int main(int argc, char **argv) {
	 ...
	 */
    vector<fs::path> song_path = find_mp3_files(argv[2], false);
	vector<fs::path> info_path = find_mp3_files(argv[2], true);
    //cout << "Found " << song_path << " songs.\n";

	string file_name = song_path[1];
	cout << file_name;
	...
	event_loop(epoll_fd, serv_sock, song_path, info_path);
}
 ...
 * 				to check.
 * @param find_info Boolean flag that either returns info or song
 *
 * @return The path of a MP3 or info file inside of the specified directory.
 */
std::vector<fs::path> find_mp3_files(const char *dir, bool find_info) {
	int num_mp3_files = 0;
	int num_info_files = 0;
	vector<fs::path> mp3_path (10); //Arraylist of mp3s and info files
	vector<fs::path> info_path (10); //Returns vector of size 1, might change to return whole filled vector
	...
	for(fs::directory_iterator entry(dir); entry != fs::directory_iterator(); ++entry) {
		...
		if (entry->path().extension() == ".mp3") {
			...
			num_mp3_files++;
			mp3_path[num_mp3_files] = entry->path(); // Each time a mp3 or info file is added, put path into its index in vector
			...
			if (fs::is_regular_file(info_file_path)) {
				num_info_files++;
				info_path[num_info_files] = info_file_path;
		...
		for (int n = 0; n < num_events; n++) {
			...
			// this socket)
				...
				if (events[n].data.fd == server_socket) {
					 ...
					 */
					clients[events[n].data.fd].handle_input(epoll_fd, song_path, info_path);
				}
            }
